% 2016.07.12 实际路上测试，3个标志点，测试仅IMU的补偿效果
clc;
clear 
close all
...
load('road_data/imu_camera_t_1-3.mat');
        ...
        if dt_current < dt_next && carmera_index < NUM_camera
            if isFirstTime == 1 % 是否是第一次进入
                ...
                camera_uv_new = camera_uv(:, carmera_index);
                 ...
                 duv_true = camera_uv_pre - camera_uv_true; % 前后两帧里面像素点的运动
                 dR = sqrt(sum(duv.^2)) ; % 预测结果的误差半径
                 dR_rotation = sqrt(sum(duv_true.^2)) ; % 由于转动导致的前后两帧的像素偏移
                 ...
                 duv_true_save(:, carmera_index-1) = duv_true;
...
%% 点坐标预测
figure()
plot(camera_uv_true_save(1,:), -camera_uv_true_save(2,:));
hold on;
grid on;
plot(camera_uv_true_save(1,1), -camera_uv_true_save(2,1),'*'); % 起始点
title('实际的点坐标在图像中的位置')
...
grid on;
hold on;
plot(duv_true_save(1, :));
plot(duv_save(2, :));
plot(duv_true_save(2, :));
% plot(camera_uv_true_save(1, :)/50);
% plot(camera_uv_true_save(2, :)/50);
% legend('dX','dY','X','Y');
legend('du-预测的误差','du-true','dv-预测的误差','dv-true');
title('XY误差')
