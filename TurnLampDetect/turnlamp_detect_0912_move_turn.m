...
close all
%% IMU 数据预处理
origin_address = ['data/0912_different/move_turn/'];
origin_name = 'move_turn-';

% data_att_IMU = [imu_time; att*180/pi; acc_cal; gyro_cal]';
address_att_IMU = [origin_address, 'data_att_IMU.mat'];
load(address_att_IMU);
data_att_C = data_att_IMU(1:4, :); % 相机的姿态
time_start = data_att_C(1, 1) - 60;
...
data_turnlamp_resample = resample(data_turnlamp(2,:), data_turnlamp(1,:), fs_HZ);

%% 计算
NUM = length(data_turnlamp_resample) - 50;
for i = 1:NUM
    att_C = data_att_C_resample(:, 1)/180*pi;
    att_C(3) = 0;
...
figure()
subplot(3,1,1)
plot(data_att_C_resample(1,:) - mean(data_att_C_resample(1,1:10)));
hold on;
plot(data_att_G_resample(1, :) - mean(data_att_G_resample(1,1:10)));
plot(data_turnlamp_resample*10);
grid on;
legend('att-X-C', 'att-X-G', 'turnlamp');

subplot(3,1,2)
plot(data_att_C_resample(2,:) - mean(data_att_C_resample(2,1:10)));
hold on;
plot(data_att_G_resample(2, :) - mean(data_att_G_resample(2,1:10)));
plot(data_turnlamp_resample*10);
grid on;
legend('att-Y-C', 'att-Y-G', 'turnlamp');

subplot(3,1,3)
plot(data_att_C_resample(3,:) - mean(data_att_C_resample(3,1:10)));
hold on;
plot(data_att_G_resample(3, :) - mean(data_att_G_resample(3,1:10)));
grid on;
