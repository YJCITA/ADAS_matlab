...
%% 车子运动轨迹解算
    while is_CAN_match_lane == 0
        ...
        if dt_CAN_lane_C < dt_CAN_lane_N 
            if dt_CAN_lane_C<0.2                
...
match_time_counter = match_time_counter + 1;
%% 坐标点预测
% IPM后的手工标注特征点在图像中的坐标
...
% 计算车道线的初值，只做一次
% if is_FirstLaneEst == 1
    for j = 1:12
        % 矩阵式逆投影变化
        uv_new = [data_uv_feature([j*2-1:j*2], k); 1];
        xy_tmp = inv(R_IPM)*uv_new;
        s_2 = h/(xy_tmp(3)); % 缩放系数（因为相机高度是固定的）
        xy_orign = xy_tmp*s_2 + Pc; % 地理坐标系下坐标
        Xk_orign(1:2, j) =xy_orign(1:2);
    end
% end
...
for i = 1:12
    Xk = Xk_orign(1:2, i); 
    X_new(1:2, i) = Rn2c_kT*(Xk - P_kT);
end
% Xk = xy_feature(1:2, 7); % 先以中间车道线的第三个点做测试
% X_new(1:2) = Rn2c_kT*(Xk - P_kT);
...
%% 直线车道拟合
    ...
    if is_FirstLaneEst == 1
        ...
        d_angle_line_M = atan(line_p(1,2)) - atan(line_p_pre(1,2));
        d_angle_line_M = d_angle_line_M*180/pi;
        line_p_pre = line_p;
    end
    ...
    for i = 1:3
        Y = X_new(2, (i*4-3):i*4)';
        X = [X_new(1, (i*4-3):i*4)' , [1 1 1 1]'];
        line_p_est(:, i) = inv(X'*X)*X'*Y;   
    end 
    
    
    % 读取图片数据
    str_data = sprintf('data/0801_tracking/1/jpg/%d.jpg',k);
    I = imread(str_data);
    I_g = rgb2gray(I);
    [m n] = size(I_g);
    CC = I_g;
