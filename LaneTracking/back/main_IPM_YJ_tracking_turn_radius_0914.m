...
% 写入txt
%     fp = fopen('F:\Develop\ADAS\Code\matlab\LaneTracking_0803\data\0902_R\car_move_radius.ini', 'wt');
...
%     str_data = sprintf('data/0902_R/frame_160025/%08d.jpg',k);
    ...
    %% 画弯道
    R_current = R_current_VNw_speed; %R_current_VN300;
    if R_current ~=0
        j = 0;  
        ...
        for x_line = x_min:0.1:x_max_t
            ...
            M = (-x_line + x_max_t)*H1/x_max_t; % 减少重复计算
    ...
    %%% R_current_steer
    %%% 画弯道
%     R_current = R_current_VN300; %R_current_steer;
%     if R_current ~=0
%         j = 0;    
%         R_current_abs = abs(R_current);
%         x_max_t = min([x_max, R_current_abs]);
